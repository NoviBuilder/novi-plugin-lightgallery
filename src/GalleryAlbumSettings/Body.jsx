...
export default class Body extends Component {
    constructor(props) {
        super(props);
        let items = this.formItemData(props.element);
        this.state = {
            items,
            active: null,
            elementsToRemove: [],
    ...
    render() {
        ...
        return this._renderItemSettings()
    }
                ...
                if (!item.thumb) {
                    ...
                    return (
                        <div key={`${item.thumb}${new Date().getTime()}-${index}`} className="lightgallery-album-item">
                            <span className="lightgallery-album-item-image"
                                  style={{backgroundImage: `url("${src}")`}}></span>
                    ...
                    <div key={`${item.src}${new Date().getTime()}-${index}`} className="lightgallery-album-item">
                        <span className="lightgallery-album-item-fallback-image">
                            <svg viewBox="0 0 20 20">
                                <path
                                    d="M7.5 15c-0.076 0-0.153-0.017-0224-0.053-0.169-0.085-0.276-0.258-0.276-0.447v-9c0-0.189 0.107-0.363 0.276-0.447s0.372-0.066 0.524 0.047l6 4.5c0.126 0.094 0.2 0.243 0.2 0.4s-0.074 0.306-0.2 0.4l-6 4.5c-0.088 0.066-0.194 0.1-0.3 0.1zM8 6.5v7l4.667-3.5-4.667-3.5z"/>
                                <path
    ...
    _renderControls(index) {
        return (
            <span className="lightgallery-album-item-hover-wrap">
                <span className="lightgallery-album-item-settings" onClick={this._openItemSettings.bind(this, index)}>
                    <svg viewBox="0 0 20 20">
                        <path
    ...
    _removeItem(index) {
        ...
        let elementsToRemove = this.state.elementsToRemove.slice();
        let htmlElement = this._removeItemHTML(items[index]);
        let captionElement = this._removeItemCaption(items[index]);
        if (htmlElement){
            elementsToRemove.push(htmlElement);
        }
        if (captionElement){
            elementsToRemove.push(captionElement);
        }
        ...
        if (items.length > 1){
            ...
            let activeItem = Object.assign({}, items[0]);
            activeItem.src = "";
            activeItem.subHtml = "";
            activeItem.type = "url";
            activeItem.thumb = "";
            activeItem.caption = "";
            delete activeItem.html;
            delete activeItem.iframe;
            items = [activeItem];
        }
        this.setState({
            items,
    ...
    _onItemAdd() {
        let items = this.state.items.slice();
        items.push({src: "", subHtml: "", type: "url"});
        this.setState({
            items
        })
    }

    _openItemSettings(index) {
        this.setState({
            active: index
        })
    }
    ...
    _renderItemSettings() {
        let item = this.state.items[this.state.active];
        let result = Utils.getLinkType(item.src);
        return (
            <div
    ...
    _onItemChange(data) {
        let items = this.state.items.slice();
        let item = Object.assign({}, items[this.state.active]);
        item.src = data.src;
    ...
    formItemData(element) {
        if (!JSON.parse(novi.element.getAttribute(element, 'data-lg-dynamic-elements'))) return null;
        let items = JSON.parse(novi.element.getAttribute(element, 'data-lg-dynamic-elements'));
        for (let i = 0; i < items.length; i++) {
            let item = items[i];
            if (item.src){
                item.type = Utils.getLinkType(item.src).type;
            }
            if (item.subHtml) {
                let captionValue = item.subHtml;
                let isSelector = true;
                try {
                ...
                if (isSelector) {
                    let captionEl = element.ownerDocument.querySelector(captionValue);
                    if (captionEl){
                        captionEl = novi.element.getStaticReference(captionEl);
                        item.caption = captionEl.innerHTML;
                        item.captionElement = captionEl;
                    }else{
            ...
            if (item.html) {
                item.type = "video";
                let videoSelector = item.html, videoElement;
                ...
                if (videoElement) {
                    if (videoElement.hasAttribute('src')) {
                        item.src = videoElement.getAttribute('src');
                    } else {
                        let source = videoElement.querySelector('source');
                        if (source) item.src = source.getAttribute("src");
                    }
                }
            }
        }
    ...
    _removeItemHTML(item){
        let videoElement = this.state.element.ownerDocument.querySelector(item.html);
        if (videoElement) {
            return videoElement;
        }
    }

    _removeItemCaption(item){
        if (item.subHtml){
            let captionValue = item.subHtml;
            let isSelector = true;
            try {
            ...
            if (isSelector) {
                let captionEl = this.state.element.ownerDocument.querySelector(captionValue);
                if (captionEl){
                    return captionEl;
                }
            }
